...
let bestScore = 0;
let bestRNA = null; //calcular a melhor RNA da rede neural dentro do treinamento
...
setInterval(() => {
    ...
    if (game.crashed) {
        ...
        dinoIndex++;
         ...
         if (dinoIndex === SAMPLES) { //checando se todos os dinos foram avaliados, como quem checa se a tarefa foi feita corretamente
              fillDinoList();
             dinoIndex = 0
             bestscore = 0
        };
    ...
    dino.score = distanceRan - 2000 // calculando a pontuação do dinossauro

    const player = { //calculando a posição do dino
        x: tRex.xPos,
        y: tRex.yPos,
        speed: currentSpeed
    };
    ...
    if (obstacle) { //calculando a distância do dino ao obstaculo para saber quando pular
        ...
        //valores bulianos (verdadeiro ou falso)    
